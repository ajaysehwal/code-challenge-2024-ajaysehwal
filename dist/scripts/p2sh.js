...
function extractScriptHash(scriptPubKey) {
...
const transaction = new transaction_1.Transaction(JSON.parse(transactionJSON1));
class P2SH {
    constructor(transaction) {
        this.transaction = transaction;
    }
    execute(scriptpubkey, redeemScript, index) {
        if (!this.verifyScriptExecution(scriptpubkey, redeemScript, index)) {
            return false;
        }
        return true;
    }
    StandardScriptExection(scriptpubkey_asm, redeemScript) {
        const stack = [];
    ...
    RedeemScriptExecution() { }
    verifyScriptExecution(scriptpubkey_asm, redeemScript, 
    // scriptSig: string,
    index) {
        ...
        const message = this.getMessageHash(this.transaction, index, key, hashTypeNumber);
    ...
    hash160(data) {
        return (0, crypto_1.createHash)("ripemd160")
            .update((0, crypto_1.createHash)("sha256").update(data).digest())
            .digest();
    }
    getMessageHash(transaction, inputIndex, publickeyhash, hashType = 0x01) {
